                        ...
                        Some(Ok(Incoming::Envelope(envelope))) => {
                            ...
                            match envelope {
                                Ok(Some(content)) => {
                                    ...
                                    {
                                        debug!("received contacts sync: blob={}, complete={:?}", contacts.blob.is_some(), contacts.complete);
                                        match state
                                            .message_receiver
                                            ...
                                            .await
                                        {
                                            Ok(contacts) => {
                                                let contacts: Vec<_> = contacts
                                                    .filter_map(|r| {
                                                        r.map_err(|e| warn!("failed to parse contact: {e}")).ok()
                                                    })
                                                    .collect();
                                                debug!("contacts retrieved: {} contacts", contacts.len());
